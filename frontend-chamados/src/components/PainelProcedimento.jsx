...
export default function PainelProcedimento({ selecionado, copiado, onCopiar, onDeletar, user }) {
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col justify-between min-h-full">
      <div>
        ...
        <div className="flex justify-between items-start gap-4 mb-6">
          ...
          <div className="flex gap-2.5 shrink-0">
            <button
              onClick={onCopiar}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-white font-semibold text-sm transition ${copiado ? 'bg-green-600' : 'bg-slate-900 hover:bg-slate-800'
                }`}
            >
      ...
      {/* Rodapé Informativo do Procedimento */}
      <div className="mt-10 pt-4 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500">
        <div className="flex items-center gap-2">
          <div className="p-1.5 bg-slate-100 rounded-full text-slate-600">
            <User size={14} />
          </div>
          <span>Criado por: <strong className="text-slate-800 font-semibold">{nomeAutor}</strong></span>
        </div>
